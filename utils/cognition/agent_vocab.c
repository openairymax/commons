...
/**
 * @file agent_vocab.c
 * @brief 执行体角色词汇表实现：角色、别名与只读集合的唯一存放点。
 *
 * 变更本表须同步 ecosystem/agents/registry/agents.yaml（声明面）。
 */
...
{
    const char *canon = agent_vocab_resolve(role);
    return canon ? canon : AGENT_VOCAB_FALLBACK;
}
