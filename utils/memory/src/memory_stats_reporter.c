...
int airy_mem_stats_reporter_init(void)
{
    ...
    /* Initialize mutex */
    int err = airy_mtx_init(&g_stats_mutex);
    if (err != 0) {
    ...
    g_shutdown_flag = 0;
    err = airy_thread_create(&g_reporter_thread, reporter_thread_func, NULL);
    if (err != 0) {
        SVC_LOG_ERROR("memory_stats_reporter: thread create failed (%d)", err);
        g_shutdown_flag = 1;
        airy_mtx_destroy(&g_stats_mutex);
        return err;
    }
...
void airy_mem_stats_reporter_shutdown(void)
{
    ...
    airy_thread_join(g_reporter_thread, NULL);
...
void airy_mem_stats_record_dealloc(size_t bytes)
{
    airy_mtx_lock(&g_stats_mutex);
...
void airy_mem_stats_record_oom(void)
{
    airy_mtx_lock(&g_stats_mutex);
    g_stats.oom_events++;
    airy_mtx_unlock(&g_stats_mutex);
