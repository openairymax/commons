...
/*
 * Generic worker thread pool implementation.
 *
 * 0.1.9 0c (IRON-6 归位)：实现从 daemons/common/src/util/thread_pool.c
 * 迁入 commons，与权威头 commons/utils/sync/include/thread_pool.h 同域；
 * 日志从 svc_logger（daemons 服务层）收敛为 commons AIRY_LOG_*。
 * daemons 侧仅保留 re-export 兼容头（include/thread_pool.h）。
 */
...
thread_pool_t *thread_pool_create(const thread_pool_config_t *config)
{
    ...
    if (!pool) {
        AIRY_LOG_ERROR("thread_pool_create: memory allocation failed for pool");
    ...
    for (uint32_t i = 0; i < num_threads; i++) {
        ...
        if (rc == 0) {
            ...
            AIRY_LOG_WARN("thread_pool_create: thread creation failed index=%u rc=%d", i, rc);
        }
    }

    if (pool->thread_count == 0) {
        AIRY_LOG_ERROR("thread_pool_create: all thread creations failed (attempted=%u)",
                       num_threads);
...
{
    if (!pool || !task) {
        AIRY_LOG_ERROR("thread_pool_submit: null parameter pool=%p task=%p", (void *)pool,
                       (void *)(uintptr_t)task);
        return AIRY_ERR_INVALID_PARAM;
    }
    if (!pool->running) {
        AIRY_LOG_ERROR("thread_pool_submit: pool not running");
        return AIRY_ERR_UNKNOWN;
    }

    task_node_t *node = (task_node_t *)AIRY_CALLOC(1, sizeof(task_node_t));
    if (!node) {
        AIRY_LOG_ERROR("thread_pool_submit: memory allocation failed for task node");
    ...
    if (pool->shutdown) {
        airy_mtx_unlock(&pool->lock);
        AIRY_FREE(node);
        AIRY_LOG_WARN("thread_pool_submit: pool is shutting down");
        return AIRY_ERR_UNKNOWN;
    }
    ...
    if (pool->queue_count >= pool->config.queue_size) {
        airy_mtx_unlock(&pool->lock);
        AIRY_FREE(node);
        AIRY_LOG_WARN("thread_pool_submit: queue full queue_count=%u queue_size=%u",
...
{
    if (!pool) {
        AIRY_LOG_ERROR("thread_pool_active_count: null pool parameter");
        return 0;
    }
    airy_mtx_lock(&pool->lock);
...
{
    if (!pool) {
        AIRY_LOG_ERROR("thread_pool_pending_count: null pool parameter");
        return 0;
    }
    airy_mtx_lock(&pool->lock);
...
{
    if (!pool) {
        AIRY_LOG_ERROR("thread_pool_is_running: null pool parameter");
        return false;
    }
    return pool->running;
}
