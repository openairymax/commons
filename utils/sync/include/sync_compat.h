...
#ifndef AIRY_RT_SYNC_COMPAT_H
#define AIRY_RT_SYNC_COMPAT_H
...
/**
 * @def AIRY_MUTEX_INIT(mutex, attrs)
 * @brief 安全互斥锁初始化宏
 */
#define AIRY_MUTEX_INIT(mutex, attrs) airy_mtx_init(mutex, attrs)

/**
 * @def AIRY_MUTEX_DESTROY(mutex)
 * @brief 安全互斥锁销毁宏
 */
#define AIRY_MUTEX_DESTROY(mutex) airy_mtx_destroy(mutex)

/**
 * @def AIRY_MUTEX_LOCK(mutex)
 * @brief 安全互斥锁锁定宏
 */
#define AIRY_MUTEX_LOCK(mutex) airy_mtx_lock(mutex)

/**
 * @def AIRY_MUTEX_TRYLOCK(mutex)
 * @brief 安全互斥锁尝试锁定宏
 */
#define AIRY_MUTEX_TRYLOCK(mutex) airy_mtx_trylock(mutex)

/**
 * @def AIRY_MUTEX_UNLOCK(mutex)
 * @brief 安全互斥锁解锁宏
 */
#define AIRY_MUTEX_UNLOCK(mutex) airy_mtx_unlock(mutex)

/**
 * @def AIRY_COND_INIT(cond, attrs)
 * @brief 安全条件变量初始化宏
 */
#define AIRY_COND_INIT(cond, attrs) airy_cond_init(cond, attrs)

/**
 * @def AIRY_COND_DESTROY(cond)
 * @brief 安全条件变量销毁宏
 */
#define AIRY_COND_DESTROY(cond) airy_cond_destroy(cond)

/**
 * @def AIRY_COND_WAIT(cond, mutex)
 * @brief 安全条件变量等待宏
 */
#define AIRY_COND_WAIT(cond, mutex) airy_cond_wait(cond, mutex)

/**
 * @def AIRY_COND_TIMEDWAIT(cond, mutex, timeout_ms)
 * @brief 安全条件变量限时等待宏
 */
#define AIRY_COND_TIMEDWAIT(cond, mutex, timeout_ms) \
    airy_cond_timedwait(cond, mutex, timeout_ms)

/**
 * @def AIRY_COND_SIGNAL(cond)
 * @brief 安全条件变量通知一个线程宏
 */
#define AIRY_COND_SIGNAL(cond) airy_cond_signal(cond)

/**
 * @def AIRY_COND_BROADCAST(cond)
 * @brief 安全条件变量通知所有线程宏
 */
#define AIRY_COND_BROADCAST(cond) airy_cond_broadcast(cond)

/**
 * @def AIRY_RWLOCK_INIT(rwlock, attrs)
 * @brief 安全读写锁初始化宏
 */
#define AIRY_RWLOCK_INIT(rwlock, attrs) airy_rwlock_init(rwlock, attrs)

/**
 * @def AIRY_RWLOCK_DESTROY(rwlock)
 * @brief 安全读写锁销毁宏
 */
#define AIRY_RWLOCK_DESTROY(rwlock) airy_rwlock_destroy(rwlock)

/**
 * @def AIRY_RWLOCK_RDLOCK(rwlock)
 * @brief 安全读锁获取宏
 */
#define AIRY_RWLOCK_RDLOCK(rwlock) airy_rwlock_rdlock(rwlock)

/**
 * @def AIRY_RWLOCK_TRYRDLOCK(rwlock)
 * @brief 安全读锁尝试获取宏
 */
#define AIRY_RWLOCK_TRYRDLOCK(rwlock) airy_rwlock_tryrdlock(rwlock)

/**
 * @def AIRY_RWLOCK_WRLOCK(rwlock)
 * @brief 安全写锁获取宏
 */
#define AIRY_RWLOCK_WRLOCK(rwlock) airy_rwlock_wrlock(rwlock)

/**
 * @def AIRY_RWLOCK_TRYWRLOCK(rwlock)
 * @brief 安全写锁尝试获取宏
 */
#define AIRY_RWLOCK_TRYWRLOCK(rwlock) airy_rwlock_trywrlock(rwlock)

/**
 * @def AIRY_RWLOCK_UNLOCK(rwlock)
 * @brief 安全读写锁解锁宏
 */
#define AIRY_RWLOCK_UNLOCK(rwlock) airy_rwlock_unlock(rwlock)

/**
 * @def AIRY_SEM_INIT(sem, pshared, value)
 * @brief 安全信号量初始化宏
 */
#define AIRY_SEM_INIT(sem, pshared, value) airy_sem_init(sem, pshared, value)

/**
 * @def AIRY_SEM_DESTROY(sem)
 * @brief 安全信号量销毁宏
 */
#define AIRY_SEM_DESTROY(sem) airy_sem_destroy(sem)

/**
 * @def AIRY_SEM_WAIT(sem)
 * @brief 安全信号量等待宏
 */
#define AIRY_SEM_WAIT(sem) airy_sem_wait(sem)

/**
 * @def AIRY_SEM_TRYWAIT(sem)
 * @brief 安全信号量尝试等待宏
 */
#define AIRY_SEM_TRYWAIT(sem) airy_sem_trywait(sem)

/**
 * @def AIRY_SEM_TIMEDWAIT(sem, timeout_ms)
 * @brief 安全信号量限时等待宏
 */
#define AIRY_SEM_TIMEDWAIT(sem, timeout_ms) airy_sem_timedwait(sem, timeout_ms)

/**
 * @def AIRY_SEM_POST(sem)
 * @brief 安全信号量发布宏
 */
#define AIRY_SEM_POST(sem) airy_sem_post(sem)

/**
 * @def AIRY_SEM_GETVALUE(sem, value)
 * @brief 安全信号量获取值宏
 */
#define AIRY_SEM_GETVALUE(sem, value) airy_sem_getvalue(sem, value)
 ...
 * 当 platform.h 已包含时，宏调用 platform.h 中的 airy_* 函数。
 */
...
#else /* AIRY_PLATFORM_H defined */

#define AIRY_MUTEX_INIT(mutex, attrs) airy_mtx_init(mutex)
#define AIRY_MUTEX_DESTROY(mutex) airy_mtx_destroy(mutex)
#define AIRY_MUTEX_LOCK(mutex) airy_mtx_lock(mutex)
#define AIRY_MUTEX_TRYLOCK(mutex) airy_mtx_trylock(mutex)
#define AIRY_MUTEX_UNLOCK(mutex) airy_mtx_unlock(mutex)

#define AIRY_COND_INIT(cond, attrs) airy_cond_init(cond)
#define AIRY_COND_DESTROY(cond) airy_cond_destroy(cond)
#define AIRY_COND_WAIT(cond, mutex) airy_cond_wait(cond, mutex)
#define AIRY_COND_TIMEDWAIT(cond, mutex, timeout_ms) \
    airy_cond_timedwait(cond, mutex, timeout_ms)
...
#define AIRY_RWLOCK_INIT(rwlock, attrs) ((void)(rwlock), (void)(attrs), 0)
#define AIRY_RWLOCK_DESTROY(rwlock) ((void)(rwlock))
#define AIRY_RWLOCK_RDLOCK(rwlock) ((void)(rwlock))
#define AIRY_RWLOCK_TRYRDLOCK(rwlock) ((void)(rwlock))
#define AIRY_RWLOCK_WRLOCK(rwlock) ((void)(rwlock))
#define AIRY_RWLOCK_TRYWRLOCK(rwlock) ((void)(rwlock))
#define AIRY_RWLOCK_UNLOCK(rwlock) ((void)(rwlock))

#define AIRY_SEM_INIT(sem, pshared, value) ((void)(sem), (void)(pshared), (void)(value), 0)
#define AIRY_SEM_DESTROY(sem) ((void)(sem))
#define AIRY_SEM_WAIT(sem) ((void)(sem))
#define AIRY_SEM_TRYWAIT(sem) ((void)(sem))
#define AIRY_SEM_TIMEDWAIT(sem, timeout_ms) ((void)(sem), (void)(timeout_ms))
#define AIRY_SEM_POST(sem) ((void)(sem))
#define AIRY_SEM_GETVALUE(sem, value) ((void)(sem), (void)(value))
 ...
 * AIRY_MUTEX_LOCK + 手动 if 检查 + AIRY_MUTEX_UNLOCK。
 */
 ...
 *
 * 类型兼容：当 platform.h 被引入（AIRY_PLATFORM_H 定义）时，mutex 字段
 * 使用 platform.h 的 airy_mtx_t（值类型：Linux=pthread_mutex_t，
 * Windows=CRITICAL_SECTION）；否则使用 sync 库的 sync_mutex_t（指针类型）。
 * 两种模式下 airy_mtx_lock/unlock 均可用，cleanup 逻辑无需区分。
 */
#ifdef AIRY_PLATFORM_H
typedef struct {
    airy_mtx_t *mutex;   /**< platform.h 模式：指向 airy_mtx_t 变量 */
 ...
 *
 * @param m 互斥锁变量（airy_mtx_t 或 sync_mutex_t 类型，宏内部取地址 &m）
 *
 ...
 * 使用 MSVC 时需在函数返回前手动调用 AIRY_MUTEX_UNLOCK。
 */
