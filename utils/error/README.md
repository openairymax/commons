...
- **统一错误码**：所有错误码为负值，成功为 0（`AIRY_OK`），分段管理避免冲突
...
### airy_error_chain_t — 错误链

| 字段 | 类型 | 说明 |
|------|------|------|
| `code` | `airy_err_t` | 错误码 |
| `depth` | `int` | 当前链深度 |
| `contexts` | `airy_error_context_entry_t[16]` | 错误上下文条目数组 |

### airy_error_context_entry_t — 错误上下文条目
...
| 函数 | 说明 |
|------|------|
| `airy_error_str(code)` | 获取错误码的可读描述 |
| `airy_error_get_severity(code)` | 获取错误码的严重程度 |
| `airy_error_get_chain()` | 获取当前线程的错误链 |
| `airy_error_clear()` | 清除当前线程的错误链 |
| `airy_error_push_ex(code, file, line, func, fmt, ...)` | 推送错误上下文到错误链 |
| `airy_error_print_chain(chain)` | 打印错误链（调试用） |
| `airy_error_chain_to_json(chain)` | 将错误链转为 JSON 字符串 |

### 错误链增强

| 函数 | 说明 |
|------|------|
| `airy_error_chain_iter_init(chain, iter)` | 初始化错误链迭代器 |
| `airy_error_chain_iter_next(iter)` | 获取下一个错误上下文条目 |
| `airy_error_chain_get_depth(chain)` | 获取错误链深度 |
| `airy_error_chain_get_root_error(chain)` | 获取最早的错误码 |
| `airy_error_chain_get_latest_error(chain)` | 获取最新的错误码 |
| `airy_error_chain_format(chain, lang)` | 格式化错误链为可读字符串 |

### 多语言支持

| 函数 | 说明 |
|------|------|
| `airy_error_set_language(lang)` | 设置当前语言环境 |
| `airy_error_get_language()` | 获取当前语言环境 |
| `airy_error_str_i18n(code, lang)` | 获取错误码的本地化描述 |
| `airy_error_register_i18n(entries, count)` | 注册自定义错误码的本地化描述 |
...
| `AIRY_CHECK(cond, code, msg)` | 条件检查，失败时返回错误 |
...
| 函数 | 说明 |
|------|------|
| `airy_error_get_stats(stats)` | 获取全局错误统计 |
| `airy_error_reset_stats()` | 重置全局错误统计 |
...
// 错误传播
airy_err_t caller() {
    AIRY_PROPAGATE(do_something(NULL));
...
if (chain->depth > 0) {
    airy_error_print_chain(chain);
    char *json = airy_error_chain_to_json(chain);
...
| 依赖 | 说明 |
|------|------|
| `types/include/types.h` | 基础类型定义（`airy_timestamp_t` 等） |
| `stddef.h` | 标准类型定义 |
| `stdint.h` | 固定宽度整数类型 |
