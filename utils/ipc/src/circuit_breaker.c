 ...
 *
 * 0.1.9 0c (IRON-6 归位 + SSoT 收敛)：实现从
 * daemons/common/src/util/circuit_breaker.c 迁入 commons/utils/ipc，
 * 与权威头 commons/utils/ipc/include/circuit_breaker.h 同域；日志/内存/
 * 字符串原语统一收敛 commons 层；daemons/common 仅留 re-export 兼容头；
 * atoms/syscall 的 airy_sys_cbrk_* 副本零调用零声明，判定死代码删除；
 * cupolas 版为独立 call-with-timeout/registry 语义（见 cupolas/
 * src/circuit_breaker.h），API 不重叠，保留。
 *
                                        ...
                                        char *fallback_result, size_t result_size)
{
    ...
    airy_err_t err = AIRY_ERR_UNKNOWN;
