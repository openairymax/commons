...
static void test_sanitize_sql_identifier(void **state)
{
    ...
    AIRY_TEST_ASSERT_STRING_EQUAL("valid_table", sanitized);
    if (sanitized)
        free(sanitized);
    sanitized = NULL;
...
static void test_sanitize_shell_param(void **state)
{
    ...
    // 测试安全参数
    err = airy_sanitize_shell_param("safe_filename.txt", &sanitized);
    AIRY_TEST_ASSERT_SUCCESS(err);
    AIRY_TEST_ASSERT_PTR_NOT_NULL(sanitized);
    if (sanitized)
        free(sanitized);
...
static void test_parse_url(void **state)
{
    ...
    AIRY_TEST_ASSERT_PTR_NOT_NULL(path);
    AIRY_TEST_ASSERT_STRING_EQUAL("https", scheme);
    AIRY_TEST_ASSERT_STRING_EQUAL("www.example.com", host);
    AIRY_TEST_ASSERT_INT_EQUAL(8080, port);
...
static void test_safe_memcpy(void **state)
{
    ...
    // 测试缓冲区溢出保护
    err = airy_safe_memcpy(dest, 5, src, strlen(src) + 1);
...
static void test_safe_strcat(void **state)
{
    ...
    // 测试缓冲区溢出保护
    err = airy_safe_strcat(dest, sizeof(dest), " This is a very long string that will overflow");
    AIRY_TEST_ASSERT_FALSE(err == AIRY_OK);
}
...
static void test_validation_macros(void **state)
{
    ...
    AIRY_TEST_ASSERT_SUCCESS(err);
    AIRY_TEST_ASSERT_STRING_EQUAL("test", dest);
}
