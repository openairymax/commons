...
static void test_safe_int_operations(void **state)
{
    ...
    // 安全加法
    AIRY_TEST_ASSERT_SUCCESS(safe_add_int(100, 200, &result));
    AIRY_TEST_ASSERT_INT_EQUAL(300, result);

    // 加法溢出检测
    AIRY_TEST_ASSERT_FALSE(safe_add_int(INT_MAX, 1, &result) == 0);

    // 减法溢出检测（通过加法模拟）
    AIRY_TEST_ASSERT_FALSE(safe_add_int(INT_MIN, -1, &result) == 0);

    // 安全乘法
    AIRY_TEST_ASSERT_SUCCESS(safe_mul_int(100, 100, &result));
    AIRY_TEST_ASSERT_INT_EQUAL(10000, result);

    // 乘法溢出检测
    AIRY_TEST_ASSERT_FALSE(safe_mul_int(INT_MAX, 2, &result) == 0);
}
...
static void test_safe_size_operations(void **state)
{
    ...
    // 安全加法
    AIRY_TEST_ASSERT_SUCCESS(safe_add_size(1000, 2000, &result));
    AIRY_TEST_ASSERT_TRUE(result == 3000);

    // 加法溢出检测
    AIRY_TEST_ASSERT_FALSE(safe_add_size(SIZE_MAX, 1, &result) == 0);

    // 安全乘法
    AIRY_TEST_ASSERT_SUCCESS(safe_mul_size(1024, 1024, &result));
    AIRY_TEST_ASSERT_TRUE(result == 1048576);

    // 乘法溢出检测
    AIRY_TEST_ASSERT_FALSE(safe_mul_size(SIZE_MAX, 2, &result) == 0);
}
...
{
    AIRY_TEST_ASSERT_INT_EQUAL(0, (int)safe_strlen(NULL));
    AIRY_TEST_ASSERT_INT_EQUAL(0, (int)safe_strlen(""));
    AIRY_TEST_ASSERT_INT_EQUAL(5, (int)safe_strlen("Hello"));
    AIRY_TEST_ASSERT_INT_EQUAL(11, (int)safe_strlen("Hello World"));
}
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_safe_strcpy),          cmocka_unit_test(test_safe_strcat),
        cmocka_unit_test(test_safe_memcpy),          cmocka_unit_test(test_safe_int_operations),
        cmocka_unit_test(test_safe_size_operations), cmocka_unit_test(test_array_access_safety),
        cmocka_unit_test(test_ptr_offset_safety),    cmocka_unit_test(test_type_conversion_safety),
        cmocka_unit_test(test_safe_strlen),          cmocka_unit_test(test_safe_strcmp),
    };
